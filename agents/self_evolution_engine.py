...
def _save_approved_lessons(lesson_details: list[dict]) -> int:
    """Auto-save only save_now + low-risk + confidence>=0.75 lessons."""
    saved = 0
...
def run(
    ...
    # Step 7: Save only approved lessons
    lesson_details = output.get("lesson_details", [])
    saved_count = _save_approved_lessons(lesson_details)
    if saved_count:
