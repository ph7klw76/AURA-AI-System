...
def validate_json_schema(result: Dict[str, Any], json_schema: Optional[Dict[str, Any]]) -> None:
    ...
    When the env var ``COSCIENTIST_LENIENT_SCHEMA`` is set (e.g. for models
    that do not support native ``response_format``), schema validation is
    relaxed: missing required fields and extra properties are logged but do
    NOT cause a validation failure.  This lets the workflow complete even
    when the model cannot produce structured output natively.
    ...
    Raises:
        ValidationError: If the result doesn't match the schema (unless lenient mode)
    """
    ...
    # In lenient mode, strip constraints that models without structured
    # output cannot reliably satisfy.
    if _lenient and isinstance(actual_schema, dict):
        actual_schema = _make_lenient_schema(actual_schema)
...
def _make_lenient_schema(schema: dict) -> dict:
    """Return a copy of *schema* with ``required`` and ``additionalProperties``
    restrictions removed so that partial / extra-keyed JSON still validates."""
    import copy as _copy
    s: dict = _copy.deepcopy(schema)
    s.pop("required", None)
    s.pop("additionalProperties", None)
    if "properties" in s and isinstance(s["properties"], dict):
        for _k, _v in s["properties"].items():
            if isinstance(_v, dict):
                _v.pop("required", None)
                _v.pop("additionalProperties", None)
    return s
...
async def call_llm(
    ...
    try:
        ...
        try:
            ...
            if "response_format" not in str(e) or "response_format" not in completion_args:
                raise  # unrelated — re-raise immediately
            logger.info(
                "response_format rejected by %s — retrying with prompt-based JSON instruction",
                model_name,
            )
            completion_args.pop("response_format", None)
            _json_hint = (
                "\n\n--- IMPORTANT ---\n"
        ...
        raise  # already handled or unrecoverable
    except Exception as e:
