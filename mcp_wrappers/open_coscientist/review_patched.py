...
async def review_node(state: WorkflowState) -> Dict[str, Any]:
    ...
    if invalid_reviews:
        ...
        if _lenient:
            logger.warning(
                "lenient mode: %d/%d reviews invalid — creating fallback reviews",
                len(invalid_reviews), len(reviews),
            )
            for i in invalid_reviews:
                dummy = reviews[i]
                dummy.review_summary = "Lenient fallback review (model without structured output)"
                dummy.overall_score = 5.0
                dummy.methodology_score = 5.0
                dummy.novelty_score = 5.0
                dummy.feasibility_score = 5.0
                dummy.mechanistic_score = 5.0
                dummy.confidence = 0.3
        else:
