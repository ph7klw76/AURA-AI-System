"""Test memory-service disabled-by-default behavior."""
...
from core.memory_service.retrieval import retrieve_relevant_memories as _retrieve
from core.memory_service.schemas import MemoryRetrievalRequest, MemoryRetrievalResult


class TestDisabledByDefault:
    """When AURA_MEMORY_SERVICE_ENABLED is unset or 0, all operations
    are no-ops and return empty results."""
    ...
    def test_retrieval_without_config_returns_empty(self):
        """Using the retrieval module directly with disabled config."""
        result = _retrieve("test", session_id="s3")
        assert result.ok is True
        assert len(result.memories) == 0
        assert result.degraded is False

    def test_enabled_when_set_to_0_stays_disabled(self):
